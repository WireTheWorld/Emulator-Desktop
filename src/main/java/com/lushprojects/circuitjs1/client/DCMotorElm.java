...
class DCMotorElm extends CircuitElm {
    ...
    // 机电参数
    double K, Kb, J, b, gearRatio, tau; //tau 保留用于静态摩擦参数化  
    public double angle;
    public double speed;
    ...
    void stamp() {
	// 添加一系列内部器件来帮助我们模拟电机。最好在代码中模拟这个微电路，
	// 以减小矩阵的规模。
	
	//nodes[0] 和 nodes[1] 是外部节点
	//电气部分：
	// 电感从电机节点 nodes[0] 到内部节点 nodes[2]
	ind.stamp(nodes[0], nodes[2]);
	// 电阻从内部节点 nodes[2] 到内部节点 nodes[3] // 电机引脚 2
	sim.stampResistor(nodes[2], nodes[3], resistance);
	// 反电动势电压源从内部节点 nodes[3] 到外部节点 nodes[1]
	sim.stampVoltageSource(nodes[3],nodes[1], voltSources[0]); // 

	//机械部分：
	// 惯性电感从内部节点 nodes[4] 到内部节点 nodes[5]
